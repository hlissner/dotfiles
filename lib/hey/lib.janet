...
(defn echo [& args]
  (def args (array ;args))  # take-while! empties the flags off the front
  (var format :raw)
  (var color nil)
  (var output stdout)
...
# computing this at load time would freeze the build's paths into the binary.
(def- *exec-path*
  (delay
    (distinct
     (filter
     ...
     #
     # The first only resolves when run out of a jpm tree (i.e. `janet bin/hey`
