...
- After deploying the Caelestia wallpaper/launcher fix, remove any unmanaged duplicate quickshell instance, restart `caelestia-shell.service`, and confirm `hyprctl layers -j` shows one Caelestia background owner, no `swaybg`, and launcher Enter starts Zen.
- If `/home/c1/the-great-sage.jpg` still exceeds Caelestia/Qt image decode limits in live UI paths, create a follow-up to generate a display-sized wallpaper asset or patch Caelestia image handling while preserving Caelestia as wallpaper owner.
