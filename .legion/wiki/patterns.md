...
Keep a small local NixOS integration module as the repository boundary: install the shell and CLI package, seed minimal `caelestia/shell.json` defaults into a mutable user file, start `caelestia-shell.service` under `hyprland-session.target`, and keep reload/restart hooks inside the repo's existing session ownership model.
