...
- Final `nix build --impure --dry-run .#nixosConfigurations.ramen.config.system.build.toplevel` reaches system build evaluation and is blocked only by the pre-existing agenix assertion: no host key at `/etc/ssh/host_ed25519`.
