# Axiom Quickshell Quick Controls and OSD - 任务清单
...
- 执行模式：中风险默认实现模式。
- 范围：`dots-hyprland-desktop-rfc` Stage 3 only；实现 quick controls and OSD。
- 风险边界：保留 fallback tools 和 existing media-key behavior；不进入 Stage 4/5。

## 阶段 1: Contract ✅ COMPLETE

- [x] 恢复 parent RFC Stage 3 和当前 Stage 1/2 状态 | 验收: 已确认 notification/search 基线和 OSD/fallback 现状
- [x] 创建 Stage 3 task contract | 验收: `plan.md` 覆盖 goal/scope/acceptance/non-goals/risks
- [x] 回读 `plan.md` 和 `tasks.md` | 验收: contract 字段完整且不是摘要骨架

## 阶段 2: RFC ✅ COMPLETE

- [x] 写入 task-local RFC | 验收: 覆盖 quick controls sections、helper/status model、OSD IPC/wrapper、fallback、verification and rollback
...
- [x] 实现 Quickshell quick controls and OSD | 验收: 满足 `plan.md` 验收标准
...
- [x] 执行 `legion-wiki` 写回 | 验收: wiki 记录 Stage 3 当前结论、风险和复用模式
